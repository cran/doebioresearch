#' @title Analysis of Factorial Randomized Block Design for 3 factors
...
#' @return ANOVA, interpretation of ANOVA, R-square, normality test result, SEm, SEd and multiple comparison test result for the factors as well as the interaction.
#' @export
#' @examples
#' data(factorialdata)
...
frbd3fact<-function(data,replicationvector,fact.A,fact.B,fact.C,Multiple.comparison.test){
  frbd.3fact1<-function(dependent.var,replicationvector,fact.A,fact.B,fact.C,mean.comparison.test){
    ...
    model<-lm(dependent.var~replicationvector+fact.A+fact.B+fact.C+fact.A:fact.B+fact.A:fact.C+fact.B:fact.C+fact.A:fact.B:fact.C)
    ...
    SEa<-sqrt(anova.model[9,3]/(r*nlevels(fact.B)*nlevels(fact.C)))
    SEb<-sqrt(anova.model[9,3]/(r*nlevels(fact.A)*nlevels(fact.C)))
    SEc<-sqrt(anova.model[9,3]/(r*nlevels(fact.A)*nlevels(fact.B)))
    SEab<-sqrt(anova.model[9,3]/(r*nlevels(fact.C)))
    SEac<-sqrt(anova.model[9,3]/(r*nlevels(fact.B)))
    SEbc<-sqrt(anova.model[9,3]/(r*nlevels(fact.A)))
    SEabc<-sqrt(anova.model[9,3]/r)
    Sea<-sqrt(2)*SEa
    Seb<-sqrt(2)*SEb
    Sec<-sqrt(2)*SEc
    Seab<-sqrt(2)*SEab
    ...
    if (anova.model[3,5]>0.05){
      ...
      multiple.comparison.B<-"The means of one or more levels of factor B are not same, so go for multiple comparison test"
    }
    ...
    if (mean.comparison.test == 2){
      m.test1A<-duncan.test(dependent.var,fact.A,anova.model[9,1],anova.model[9,3])
      m.testA<-list(m.test1A$statistics,m.test1A$duncan,m.test1A$groups)
      m.test1B<-duncan.test(dependent.var,fact.B,anova.model[9,1],anova.model[9,3])
      m.testB<-list(m.test1B$statistics,m.test1B$duncan,m.test1B$groups)
      m.test1C<-duncan.test(dependent.var,fact.C,anova.model[9,1],anova.model[9,3])
      m.testC<-list(m.test1C$statistics,m.test1C$duncan,m.test1C$groups)
      m.test1AB<-duncan.test(dependent.var,fact.A:fact.B,anova.model[9,1],anova.model[9,3])
      m.testAB<-list(m.test1AB$statistics,m.test1AB$duncan,m.test1AB$groups)
      m.test1AC<-duncan.test(dependent.var,fact.A:fact.C,anova.model[9,1],anova.model[9,3])
      m.testAC<-list(m.test1AC$statistics,m.test1AC$duncan,m.test1AC$groups)
      m.test1BC<-duncan.test(dependent.var,fact.B:fact.C,anova.model[9,1],anova.model[9,3])
      m.testBC<-list(m.test1BC$statistics,m.test1BC$duncan,m.test1BC$groups)
      m.test1ABC<-duncan.test(dependent.var,fact.A:fact.B:fact.C,anova.model[9,1],anova.model[9,3])
      m.testABC<-list(m.test1ABC$statistics,m.test1ABC$duncan,m.test1ABC$groups)
    }
    if (mean.comparison.test == 3){
      m.test1A<-HSD.test(dependent.var,fact.A,anova.model[9,1],anova.model[9,3])
      m.testA<-list(m.test1A$statistics,m.test1A$parameters,m.test1A$groups)
      m.test1B<-HSD.test(dependent.var,fact.B,anova.model[9,1],anova.model[9,3])
      m.testB<-list(m.test1B$statistics,m.test1B$parameters,m.test1B$groups)
      m.test1C<-HSD.test(dependent.var,fact.C,anova.model[9,1],anova.model[9,3])
      m.testC<-list(m.test1C$statistics,m.test1C$parameters,m.test1C$groups)
      m.test1AB<-HSD.test(dependent.var,fact.A:fact.B,anova.model[9,1],anova.model[9,3])
      m.testAB<-list(m.test1AB$statistics,m.test1AB$parameters,m.test1AB$groups)
      m.test1AC<-HSD.test(dependent.var,fact.A:fact.C,anova.model[9,1],anova.model[9,3])
      m.testAC<-list(m.test1AC$statistics,m.test1AC$parameters,m.test1AC$groups)
      m.test1BC<-HSD.test(dependent.var,fact.B:fact.C,anova.model[9,1],anova.model[9,3])
      m.testBC<-list(m.test1BC$statistics,m.test1BC$parameters,m.test1BC$groups)
      m.test1ABC<-HSD.test(dependent.var,fact.A:fact.B:fact.C,anova.model[9,1],anova.model[9,3])
      m.testABC<-list(m.test1ABC$statistics,m.test1ABC$parameters,m.test1ABC$groups)
    }
    if (mean.comparison.test == 1){
      m.test1A<-LSD.test(dependent.var,fact.A,anova.model[9,1],anova.model[9,3])
      m.testA<-list(m.test1A$statistics,m.test1A$groups)
      m.test1B<-LSD.test(dependent.var,fact.B,anova.model[9,1],anova.model[9,3])
      m.testB<-list(m.test1B$statistics,m.test1B$groups)
      m.test1C<-LSD.test(dependent.var,fact.C,anova.model[9,1],anova.model[9,3])
      m.testC<-list(m.test1C$statistics,m.test1C$groups)
      m.test1AB<-LSD.test(dependent.var,fact.A:fact.B,anova.model[9,1],anova.model[9,3])
      m.testAB<-list(m.test1AB$statistics,m.test1AB$groups)
      m.test1AC<-LSD.test(dependent.var,fact.A:fact.C,anova.model[9,1],anova.model[9,3])
      m.testAC<-list(m.test1AC$statistics,m.test1AC$groups)
      m.test1BC<-LSD.test(dependent.var,fact.B:fact.C,anova.model[9,1],anova.model[9,3])
      m.testBC<-list(m.test1BC$statistics,m.test1BC$groups)
      m.test1ABC<-LSD.test(dependent.var,fact.A:fact.B:fact.C,anova.model[9,1],anova.model[9,3])
  ...
  for (j in 1:colnumber){
    output[[j]]<-frbd.3fact1(fiftn[[j]],replicationvector,fact.A,fact.B,fact.C,Multiple.comparison.test)
  }
